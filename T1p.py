...
def t1rho_model(t, I0, T1rho):
    return I0 * np.exp(-t / T1rho)

def fit_t1rho(data1, name_map1, data2=None, name_map2=None):
    t1rho_values_1, t1rho_errors_1, res_ids_1 = [], [], []
    t1rho_values_2, t1rho_errors_2, res_ids_2 = [], [], []
    ...
    for i, res in enumerate(residues):
        try:
            ...
            if res in data1:
                ...
                # Ensure you add the (1, 1) point if it's missing
                ...
                if len(pts1) >= 3:
                    ...
                    if all(i > 0 for i in i1):
                        popt1, pcov1 = curve_fit(t1rho_model, t1, i1, p0=(max(i1), 20))
                        t_fit = np.linspace(min(t1), max(t1), 100)
                        # Calculate the error (standard deviation) for T1ρ in Dataset 1
                        stddev_T1rho1 = np.sqrt(np.diag(pcov1))[1]
                        # Add both T1ρ and its error to the legend
                        ax.plot(t_fit, t1rho_model(t_fit, *popt1), '-', color='tab:blue', 
                                label=f'T1ρ={popt1[1]:.2f} \n       ± {stddev_T1rho1:.2f}')
                        
                        # Dataset 1
                        t1rho_values_1.append(popt1[1])
                        t1rho_errors_1.append(stddev_T1rho1)
            ...
            if data2 and res in data2:
                ...
                if len(pts2) >= 3:
                    ...
                    if all(i > 0 for i in i2):
                        popt2, pcov2 = curve_fit(t1rho_model, t2, i2, p0=(max(i2), 20))
                        if t_fit is None:
                            t_fit = np.linspace(min(t2), max(t2), 100)
                        # Calculate the error (standard deviation) for T1ρ in Dataset 2
                        stddev_T1rho2 = np.sqrt(np.diag(pcov2))[1]
                        # Add both T1ρ and its error to the legend
                        ax.plot(t_fit, t1rho_model(t_fit, *popt2), '-', color='tab:red', 
                                label=f'T1ρ={popt2[1]:.2f} \n       ± {stddev_T1rho2:.2f}')

                        # Dataset 2
                        t1rho_values_2.append(popt2[1])
                        t1rho_errors_2.append(stddev_T1rho2)
            ...
            if t_fit is not None:
                ...
                # Add the legend with T1ρ and the error
                ax.legend(loc='best', fontsize=6)

                # ax.set_xlim(-5, 205)
                ax.set_xticks(np.linspace(0, 200, 3))
                ax.set_ylim(-0.05, 1.05)

        except Exception as e:
            print(f"⚠️ T1ρ fit failed for residue {res}: {e}")
    ...
    fig.suptitle(fr"$T_1ρ$ Fits per Residue", fontsize=14)

    plt.show()

    return {
    "res_ids_1": res_ids_1,
    "t1rho_values_1": t1rho_values_1,
    "t1rho_errors_1": t1rho_errors_1,
    "res_ids_2": res_ids_2,
    "t1rho_values_2": t1rho_values_2,
    "t1rho_errors_2": t1rho_errors_2
}

def plot_t1rho(fit_result, sequence=None, psipred_file=None, y_limit=None, x_limit=None, save=False, skip_nterm=0):
    res_ids_1 = fit_result['res_ids_1']
    values_1 = fit_result['t1rho_values_1']
    errors_1 = fit_result['t1rho_errors_1']
    res_ids_2 = fit_result['res_ids_2']
    values_2 = fit_result['t1rho_values_2']
    errors_2 = fit_result['t1rho_errors_2']
    ...
    ax.set_title(fr"$T_1ρ$ Relaxation Times per Residue", y=1.16 if sequence and psipred_file else 1.08 if sequence else 1)
    ax.legend()
    plt.tight_layout()

    if save:
        name = sanitise_filename("T1rho_summary")
        plt.savefig(f"{name}_T1rho.svg")
    else:
        plt.show()

def calculate_r1rho(t1rho_values, t1rho_errors):
    r1rho_values = (1 / np.array(t1rho_values)) * 1000
    r1rho_errors = (np.array(t1rho_errors) / (np.array(t1rho_values) ** 2)) * 1000
    return r1rho_values, r1rho_errors

def plot_r1rho(fit_result, sequence=None, psipred_file=None, y_limit=None, x_limit=None, save=False, skip_nterm=0):
    res_ids_1 = fit_result['res_ids_1']
    values_1 = fit_result['t1rho_values_1']
    errors_1 = fit_result['t1rho_errors_1']
    res_ids_2 = fit_result['res_ids_2']
    ...
    # Get R1ρ values and errors
    r1rho_values_1, r1rho_errors_1 = calculate_r1rho(values_1, errors_1)
    r1rho_values_2, r1rho_errors_2 = calculate_r1rho(values_2, errors_2)

    # Combine both datasets' R1ρ values to determine global max for y-axis scaling
    all_values = np.concatenate([r1rho_values_1, r1rho_values_2])
    ...
    ax.set_xlabel('Residue')
    ax.set_ylabel(r'$R_1\rho\;(\mathrm{s}^{-1})$')
    if sequence:
        tick_positions = np.arange(0, len(sequence), 10)
        ax.set_xticks(tick_positions)
    ax.xaxis.set_major_formatter(ticker.FuncFormatter(lambda x, _: int(x - skip_nterm)))
    ax.xaxis.set_minor_locator(AutoMinorLocator(10))
    ax.set_title(fr"$R_1ρ$ Relaxation Rates per Residue", y=1.16 if sequence and psipred_file else 1.08 if sequence else 1)
    ax.legend()
    ...
    if save:
        name = sanitise_filename("R1rho_summary")
        plt.savefig(f"{name}_R1rho.svg")
    else:
        plt.show()
...
def main():
    ...
    fit_result = fit_t1rho(residue_intensity_data1, id_to_name_mapping1, residue_intensity_data2, id_to_name_mapping2)
