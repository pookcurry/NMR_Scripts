...
def fit_cpmg(tau, data1, name_map1, data2=None, name_map2=None):
    if data2:
    ...
    for i, res in enumerate(residues):
        try:
            ...
            if res in data1:
                ...
                # Ensure you add the (1, 1.0) point if it's missing
                ...
                if len(pts1) >= 3:
                    ...
                    # If tau is available, adjust t1 values by subtracting tau
                    if tau:
                        t1 = np.array(t1) - tau
                    
                    if all(i > 0 for i in i1):
                        popt1, pcov1 = curve_fit(cpmg_model, t1, i1, p0=(max(i1), 20))
                        t_fit = np.linspace(min(t1), max(t1), 100)
                        # Calculate the error (standard deviation) for T2 in Dataset 1
                        stddev_T21 = np.sqrt(np.diag(pcov1))[1]
                        ax.plot(t1 + tau, i1, '-o', color='tab:blue', markersize=4)  # Adjust for tau when plotting
                        # ax.plot(t_fit + tau, cpmg_model(t_fit, *popt1), '-', color='tab:blue', label=f'T2={popt1[1]:.2f} ± {stddev_T21:.2f}')
            ...
            if data2 and res in data2:
                ...
                if len(pts2) >= 3:
                    ...
                    # If tau is available, adjust t2 values by subtracting tau
                    if tau:
                        t2 = np.array(t2) - tau
                    
                    if all(i > 0 for i in i2):
                        popt2, pcov2 = curve_fit(cpmg_model, t2, i2, p0=(max(i2), 20))
                        if t_fit is None:
                            t_fit = np.linspace(min(t2), max(t2), 100)
                        # Calculate the error (standard deviation) for T2 in Dataset 2
                        stddev_T22 = np.sqrt(np.diag(pcov2))[1]
                        ax.plot(t2 + tau, i2, '-o', color='tab:red', markersize=4)  # Adjust for tau when plotting
                        # ax.plot(t_fit + tau, cpmg_model(t_fit, *popt2), '-', color='tab:red', label=f'T2={popt2[1]:.2f} ± {stddev_T22:.2f}')
            ...
            if t_fit is not None:
                ...
                # Add the legend with T2 and the error
                # ax.legend(loc='best', fontsize=6)

                ax.set_xlim(-5, 65)
                ax.set_xticks(np.linspace(0, 60, 3))
                # Combine data from both datasets if present
                all_y = []
                if res in data1:
                    all_y.extend(i1)
                if data2 and res in data2:
                ...
                if all_y:
                    ymin, ymax = min(all_y), max(all_y)
                    ypad = 2 * (ymax - ymin) if ymax != ymin else 0.05
                    ax.set_ylim(ymin - ypad, ymax + ypad)

        except Exception as e:
            print(f"⚠️ T2 fit failed for residue {res}: {e}")
    ...
    fig.suptitle("CPMG per Residue", fontsize=14)
    plt.show()
